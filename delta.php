  		...
  		foreach ($params as $field) {
			...
			###############################################################################
       		...
       		$oldvalue 	= civicrm_api3('Contact', 'getvalue', [
       			'id'     => $field['entity_id'],	
       			// ik denk dat dit alleen klopt bij contact id en niet als het part id is
...
//			wachthond($extdebug,1, "newsec  ($newsec)", 		$fieldname);
			$newdate 	= date("Y-m-d H:i:s", $newsec);
//			wachthond($extdebug,1, "newdate ($newdate)", 		$fieldname);
...
#		wachthond($extdebug,2, "changes['old'] c", 	$changes['old']);
#		wachthond($extdebug,2, "changes['new'] c", 	$changes['new']);
